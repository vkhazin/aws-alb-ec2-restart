...
class AwsLambdaTests(unittest.TestCase):
  ...
  def testParseSnsMessages(self):
    event = json.loads(eventString)
    snsMessages = awsLambda.parseEvent(event)
    for unhealthyTargetGroup in awsLambda.parseSnsMessages(snsMessages):
    	self.assertNotEqual(len(unhealthyTargetGroup), 0)
  
  def testFindUnhealthyTargets(self):
    event = json.loads(eventString)
    snsMessages = awsLambda.parseEvent(event)
    for snsMessage in awsLambda.parseSnsMessages(snsMessages):
			unhealthyTargets = awsLambda.findUnhealthyTargets(snsMessage['TargetGroups'])
			self.assertNotEqual(len(unhealthyTargets), 0)
